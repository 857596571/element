		  ...
		  >
		    <template slot="title">
				<div  class="navigateMenu" v-if="v.resIconNo"> 
				 ...
				 <div  class="navigateMenu" v-else> 
				      <img :src="defaultPic" class="listImgMenu"/>
				  </div>
		    ...
		    >
		      <p class="hover_secondMenu">
							<div class="hover_secondMenu_left">
								<span>{{vv.name?vv.name:vv.res_na}}</span>&nbsp;&gt;</div>
							<div class="hover_secondMenu_Right">
								<span
								class="hoverColor"
								@click.stop="see(vvv)"
								...
								v-for="(vvv,iii) in vv.children?vv.children:vv.childsList"
								:key="iii+'-three'"
							>{{vvv.name?vvv.name:vvv.res_na,vv.children}}</span>
							</div>
						</p>
		    </div>
		  </el-submenu>
			...
			<!-- <div  class="navigateMenu"> 
			  ...
			  <el-tooltip placement="right">
				 ...
				 <div  class="navigateMenu"> 
				     <img src="../img/u9.png" class="listImgMenu"/>
				 </div>
			 </el-tooltip>
			  </div>
			   <div  class="navigateMenu">
			       <img src="../img/u10.png" class="listImgMenu" @click="tapClick2"/>
			  </div>
			  </div>
			</div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      redirctUrl: localStorage.getItem('redirctUrl'),
      defaultPic: ''
    };
  },
  methods: {
    defalutPic() {
      if (this.myUserinfo) {
        let parmas = {};
        parmas.prn = this.data.JID;
        this.$http.post(this.data.defaultPic, parmas, { emulateJSON: true }).then(res => {
          this.defaultPic = res.data.obj[0].default_01;
        });
      }
    },
    mouseenter() {
