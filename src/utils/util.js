...
export const searchMatch = {
  ...
  convertArr: function(str) {
    ...
    for (var i = 0; i < arr.length; i++) {
      temp = this.getPinYin(arr[i]);
      if (temp) {
        fullFight.push(temp);
        initials.push(temp.charAt(0));
      }
    }
    return [fullFight.join(''), initials.join(''), str];
  },
  isMatch: function(keyword, convertArr) {
    // convertArr => [全拼，首字母，原文]
    var isFind = false;
    if (keyword && convertArr && convertArr.length) {
      for (var i = 0, len = convertArr.length; i < len; i++) {
        if (convertArr[i].indexOf(keyword) >= 0) {
          isFind = true;
          break;
        }
      }
